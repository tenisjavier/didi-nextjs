...
const Article = async ({ params: { slug } }: GuiasProps) => {
  ...
  const suggestedGuidesProps = {
    ...
    title: "DiDi Artículos de Lugares para Visitar en Costa Rica",
    ...
    columns: suggestedArticles.map((article: ArticleT) => {
      return {
        title: <Link href={`/cr/articulos/${article.slug}`}>{article.title}</Link>,
        desc: article.excerpt,
        ...
        btnMode: "dark",
        btnText: "Leer Guía",
        btnLink: `/cr/articulos/${article.slug}`,
      };
    }),
  };
  return (
