...
const Guide = async ({ params: { slug } }: GuiasProps) => {
  const guide = await fetchGuideBySlug(slug, "cr");
  if (!guide) return notFound();

  const suggestedGuides = await fetchGuidesByCategory("driver", "cr");
...
export async function generateStaticParams() {
  const guides = await fetchGuidesByCategory("driver", "cr");
