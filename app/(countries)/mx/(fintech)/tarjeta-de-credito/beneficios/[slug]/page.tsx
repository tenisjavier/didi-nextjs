...
const Page = async ({ params: { slug } }: GuiasProps) => {
  ...
  const partners = (await fetchPartnersByCategory("mx", "creditCard")).filter(
  ...
  const columnSectionProps = {
    ...
    columns: partners?.map((partner: PartnerT) => {
      return {
        ...
        desc: partner.desc,
        image: partner.logo,
        pathname: `/mx/tarjeta-de-credito/beneficios/${partner.slug}`,
      };
    }),
    bgColor: "bg-white",
    gap: 10,
...
export async function generateStaticParams() {
  const partners = await fetchPartnersByCategory("mx", "creditCard");
