...
const Guide = async ({ params: { slug } }: GuiasProps) => {
  const partner = await fetchPartnerBySlug("cl", slug);
  const partners = (await fetchPartnersByCategory("cl", "didimas")).filter(
  ...
  const columnSectionProps = {
    ...
    columns: partners?.map((partner: PartnerT) => {
      return {
        ...
        desc: partner.desc,
        image: partner.logo,
        pathname: `/cl/didimas/${partner.slug}`,
      };
    }),
    bgColor: "bg-white",
    gap: 10,
...
export async function generateStaticParams() {
  const partners = await fetchPartnersByCategory("cl", "didimas");
