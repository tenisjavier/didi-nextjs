...
const Article = async ({ params: { slug } }: GuiasProps) => {
  ...
  const suggestedGuidesProps = {
    ...
    columns: suggestedArticles.items.map((article) => {
      return {
        title: <Link href={`/mx/food/blog/${article.slug}`}>{article.title}</Link>,
        desc: article.excerpt,
        ...
        btnMode: "dark",
        btnText: "Leer Guía",
        btnLink: `/mx/food/blog/${article.slug}`,
      };
    }),
  };
  return (
...
export async function generateStaticParams() {
  const articles = await fetchArticles("mx", "pay");
